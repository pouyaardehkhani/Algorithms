...
def isPrime(n):
    ...
    coef(n)
    # subtracting c[n] and addingc[0] by 1 as ( x - 1 )^n - ( x^n - 1), here we are
    # subtracting c[n] by 1 and adding 1 in expression.
    c[0] = c[0] + 1
    c[n] = c[n] - 1
    # checking all the coefficients whether they are divisible by n or not. if n is not prime,
